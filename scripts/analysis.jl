...
function load_results_df(model_outputs_file)
...
function plot_soclearn_over_u_sigmoids(final_agg_df; 
    ...
    for low_payoff in low_payoffs 
        ...
        p = plot(thisdf, x=:env_uncertainty, y=:mean_social_learner_mean, 
                 ...
                 Guide.colorkey(pos=[0.5w, -0.20h]),
                 PROJECT_THEME)

        draw(
             PDF(joinpath(figure_dir, "SL_over_u_lowpayoff=$low_payoff.pdf"), 4.25inch, 3inch), 
            p
        )
    end
end
